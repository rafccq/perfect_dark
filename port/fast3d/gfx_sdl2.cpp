...
static int32_t gfx_sdl_get_fullscreen_flag_mode(void) {
	uint32_t flags = SDL_GetWindowFlags(wnd);
    return (flags & SDL_WINDOW_FULLSCREEN) && SDL_GetWindowFullscreenMode(wnd) ? 0 : 1;
}
...
static SDL_DisplayID display_idx_to_id(int displayIndex) {
	SDL_DisplayID displayID;
	int count = 0;
	SDL_DisplayID *list = SDL_GetDisplays(&count);

	if (list == NULL || count == 0) {
		sysFatalError("No display found");
	}

	if (displayIndex < 0 || displayIndex >= count) {
		sysFatalError("Invalid display index");
	}
...
static void gfx_sdl_init(const struct GfxWindowInitSettings *set) {
    ...
    int posX = set->x;
    int posY = set->y;
    int display_in_use = display_idx_to_id(0);

	if (display_in_use == 0) {
		sysFatalError("Could not get the display:\n%s", SDL_GetError());
	}
    ...
    for (u32 i = verstart; i < verend && !ctx; ++i) {
		...
		SDL_SetStringProperty(props, SDL_PROP_WINDOW_CREATE_TITLE_STRING, set->title);
...
static void get_centered_positions_native(int32_t width, int32_t height, int32_t *posX, int32_t *posY) {
    const int disp_idx = SDL_GetDisplayForWindow(wnd);
	const SDL_DisplayMode *mode = SDL_GetCurrentDisplayMode(disp_idx);
...
static void gfx_sdl_set_closest_resolution(int32_t width, int32_t height, bool should_center) {
    const SDL_DisplayID disp_idx = SDL_GetDisplayForWindow(wnd);
	const SDL_DisplayMode *mode = SDL_GetCurrentDisplayMode(disp_idx);
    SDL_DisplayMode closest = {};
    if (SDL_GetClosestFullscreenDisplayMode(disp_idx, width, height, mode->refresh_rate, true, &closest)) {
...
static int gfx_sdl_get_swap_interval(void) {
	...
	if (!SDL_GL_GetSwapInterval(&interval)) {
		sysLogPrintf(LOG_WARNING, "SDL: failed to get vsync: %s", SDL_GetError());
	}
	return interval;
}
...
int gfx_sdl_get_display_mode(int modenum, int *out_w, int *out_h) {
    const int display_in_use = SDL_GetDisplayForWindow(wnd);
	const SDL_DisplayMode *mode = SDL_GetCurrentDisplayMode(display_in_use);
	int count = 0;
	SDL_DisplayMode **list = SDL_GetFullscreenDisplayModes(display_in_use, &count);
	if (mode && modenum >= 0 && modenum < count) {
		const SDL_DisplayMode *sdlmode = list[modenum];
		*out_w = sdlmode->w;
		*out_h = sdlmode->h;
		return 1;
	}
...
int gfx_sdl_get_num_display_modes(void) {
	...
	int count;
	SDL_GetFullscreenDisplayModes(display_in_use, &count);
